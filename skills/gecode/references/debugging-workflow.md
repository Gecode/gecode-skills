...
- Do not diagnose a correctness failure by trying different propagator posting orders. Propagators are contracting, weakly monotonic operations over the domain lattice, so correctness cannot rely on scheduler order. Ordering may change cost or the trace, and weak monotonicity can affect recomputation strength, but an order-dependent solution set indicates a propagator or lifecycle bug.
...
## Propagator Test Workflow

- Reproduce custom-propagator failures inside Gecode's native integer, set, or float test infrastructure before building a separate harness.
- Use the harness's seed-controlled randomized pruning, intermittent fixpoints, cloning, posting-before/after-assignment, disable/enable, reification, and subsumption checks to narrow the failed invariant.
- Preserve the reported seed and focused test selector while diagnosing, then add the smallest deterministic regression that captures the bug.
- Add bespoke tests only for helper code outside the harness's reach; do not simulate scheduler permutations as a substitute for the native lifecycle tests.
